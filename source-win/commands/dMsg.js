...
module.exports = {
    ...
    execute(msg) {
        ...
        var time = s[1] + `000`;
        var msgContent = s[0].slice(4 + prefixString.length);
        const descEmbed = new Discord.RichEmbed()
            .setDescription(msgContent)

        if (time === undefined || time <= 0 || isNaN(time) === true) {
            console.log(`You must set an amount of time for the message to remain that is greater than 0! Type ` + prefix + `help for usage and tips.`);
        }
        else {
